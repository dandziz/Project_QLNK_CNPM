...
class PhanHoiThuTucModel{
    ...
    public function getChiTietTamVang($matv){
        $conn = $this->connectDb();
        $querySelect = "SELECT * FROM tamvang where ma_dontt = '{$matv}'";
        $resultService = mysqli_query($conn, $querySelect);
        $SEDArr = [];
        if(mysqli_num_rows($resultService)>0){
            $bds = mysqli_fetch_all($resultService, MYSQLI_ASSOC);
    ...
    public function processTV($TV){
        $connection = $this->connectDb();
        $query = "UPDATE `tamvang` SET `xacnhan`='{$TV['xacnhan']}', `ma_taikhoan`='{$TV['ma_taikhoan']}' WHERE `ma_dontt` = '{$TV['matt']}'";
